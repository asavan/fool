...
- [ ] в функции join убрать ind и подсчитывать его внутри game. При попытке повторно вставить игрока с тем же external_id обновлять ему имя, а не добавлять новую запись.
...
- [ ] в ф-ции onStart принимать seed с сервера
- [ ] вынести все числовые задержки из engine в settings
- [ ] удалить глобальные переменные из модуля uno.js (в частности handlers, let MAX_SCORE, let roundover), удалить ф-цию resetToDefaults
- [ ] сделать локальными const handlers в server.js и в client.js
